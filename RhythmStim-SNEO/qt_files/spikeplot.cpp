...
void SpikePlot::drawAxisText()
{
    QPainter painter(&pixmap);
    painter.initFrom(this);
    const int textBoxWidth = painter.fontMetrics().horizontalAdvance("ONLY AMPLIFIER CHANNELS CAN BE DISPLAYED");
...
void SpikePlot::initializeDisplay() {
    const int textBoxWidth = fontMetrics().horizontalAdvance("+" + QString::number(yScale) + " " + QSTRING_MU_SYMBOL + "V");
